...
function buildSiteExecutionTask(summary: IntakeSummary): KickoffTask {
  const lines = [
    ...
    "8. If the completed creation job reports `isPublished: false`, an unpublished site state, or a `published-site-urls` lookup returns an empty `urls` array, publish the verified site first through the documented Wix publish endpoint and then repeat the published-site-urls lookup.",
    "9. If create or publish only returns a placeholder host or dashboard URL, do one explicit published-site-urls style lookup on the verified site id before you report siteUrl as unresolved.",
    "10. Use ListWixSites only as a fallback when the completed creation job does not expose the created site identity directly or when you still need to resolve a real siteUrl after binding the site IDs.",
    "11. Do not treat a started build job as success if wixBinding still lacks those verified identity fields.",
    "12. The main business site becomes the canonical company site in wixBinding.",
    ...
    "16. If the founder provided any public source URL, inspect that exact source directly and use it to place real copy, imagery, or collection content on the main site after binding. Do not stop at empty shell creation.",
    "17. Do not create board tasks asking for basic launch copy, imagery, or starter content while the founder-provided public source links still contain usable material. Only ask for exact missing facts that block a specific Wix mutation such as pricing, policy, or inventory values.",
    "18. After each meaningful publish or placement pass, inspect the live public URL directly. If the page still shows generic starter-template content, unrelated template brand names, fake contact info, or placeholder copy such as 'Use this space to promote the business', the task is still incomplete.",
    "19. Replace the bound site's starter-template identity and placeholder sections with founder-source-derived business content before you mark this task done.",
    "20. Do not mark this task done until the bound main site has a real non-placeholder site URL and the public page no longer reads like a generic Wix starter template, unless a concrete tooling blocker is clearly reported.",
  ];
...
function buildVibeSiteExecutionTask(summary: IntakeSummary): KickoffTask {
  const lines = [
    ...
    "8. If the completed vibe-site job reports `isPublished: false`, an unpublished state, or a `published-site-urls` lookup returns an empty `urls` array, publish the verified vibe site first through the documented Wix publish endpoint and then repeat the published-site-urls lookup.",
    "9. If the builder or publish flow only returns a placeholder host or development/editor URL, do one explicit published-site-urls style lookup on the verified vibe-site id before you report vibeSiteUrl as unresolved.",
    "10. Resolve a real non-placeholder vibeSiteUrl when tooling exposes one. If only a development/editor URL is available, record it but keep pushing on public URL resolution or clearly document the blocker.",
    "11. If Picasso creates a site container or returns a verified site id but then stalls in app-spec generation, job polling, or developer-machine startup without producing a real site pass, keep the task active, record the verified ids/status, and report the exact builder stall instead of counting that run as success.",
    "12. After each meaningful publish or placement pass, inspect the public vibe-site URL directly. If it still shows generic starter-template content, unrelated template brand names, fake contact info, or placeholder copy such as 'Use this space to promote the business', the task is still incomplete.",
    "13. Compare the public vibe site directly against the current public main site. If the vibe site still feels like the same storefront shell, same section stack, same headline structure, or same plain commerce presentation, the task is still incomplete even if both URLs are valid.",
    "14. The vibe site must be materially more expressive than the main site in tone, section framing, and overall creative direction while still staying faithful to the real business.",
    "15. Replace the experimental site's starter-template identity and placeholder sections with founder-source-derived business content before you mark this task done.",
    "16. Do not mark this task done until a real vibe site exists with its own different verified site id, real public URL, public-page content that no longer reads like a generic Wix starter template, and a clearly more distinct creative direction than the main site, or a concrete tooling blocker is clearly reported.",
  ];
