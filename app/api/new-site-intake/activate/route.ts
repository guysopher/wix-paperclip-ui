...
function buildVibeSiteExecutionTask(summary: IntakeSummary): KickoffTask {
  const lines = [
    ...
    "2. If PAPERCLIP_TASK_ID and PAPERCLIP_COMPANY_ID are present, call /api/issues/$PAPERCLIP_TASK_ID/heartbeat-context and /api/companies/$PAPERCLIP_COMPANY_ID first. Do not start by scanning ~/.paperclip folders, sqlite files, run logs, or generic MCP catalogs just to rediscover the task.",
    "3. Start the experimental Picasso site in parallel with the main site whenever Picasso tooling is available.",
    "4. On this machine, prefer the built repo-local Picasso CLI at /Users/guyso/Code/Wix/picasso-dev-tools/packages/picasso-dev-tools/dist/cjs/cli.js before scanning generic MCP catalogs. Treat returned job or operation ids as asynchronous work that must be polled to terminal state.",
