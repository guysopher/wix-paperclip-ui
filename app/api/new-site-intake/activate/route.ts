...
function buildSiteExecutionTask(summary: IntakeSummary): KickoffTask {
  const lines = [
    ...
    "3b. Resolve the real `userId` and `accountId` through `fire-console__find_user` using the exact query `guyso@wix.com`. The expected result on this machine is `userId = accountId = 0d72accb-22a2-4c0b-a347-39a02b5b5304`. Do not improvise with partial searches like `guyso`, `guy`, or the company name.",
    ...
    "7. Do not use placeholder URLs such as the generic wix.com host as the canonical siteUrl.",
    "8. As soon as the site is created and you have verified site identity, create a normal board publish task assigned to `local-board`.",
    "9. That board publish task must include the exact Studio editor link whenever you can construct or verify it. If the create response exposes html_app.instance_id and metaSiteId, use the canonical Studio format: https://<site-name>.editor.wix.com/studio/<html_app.instance_id>?metaSiteId=<metaSiteId>. If the exact editor host cannot be verified, include the best verified editor or dashboard link you do have.",
    "9a. That board publish task must also include exact metaSiteId/siteId, current publish state, and the exact click path needed to finish publish and report the resulting public URL.",
    "10. If the completed creation job reports `isPublished: false`, an unpublished site state, or a `published-site-urls` lookup returns an empty `urls` array, treat the site as created but unpublished until the board completes the publish task.",
    "11. Use ListWixSites only as a fallback when the completed creation job does not expose the created site identity directly or when you still need to resolve a real siteUrl after binding the site IDs.",
    "12. Do not treat a started build job as success if wixBinding still lacks those verified identity fields.",
    "13. The main business site becomes the canonical company site in wixBinding.",
    "14. Never overwrite wixBinding with vibe-site data.",
    "15. Keep the main site and any experimental vibe site clearly distinguished in comments and handoffs.",
    "16. Do not complete this task with architecture-only recommendations if no main site is bound yet. The only acceptable non-build outcome is a concrete tooling failure after real creation attempts.",
    "17. If the founder provided any public source URL, inspect that exact source directly and use it to place real copy, imagery, or collection content on the main site after binding. Do not stop at empty shell creation.",
    "18. Do not create board tasks asking for basic launch copy, imagery, or starter content while the founder-provided public source links still contain usable material. Only ask for exact missing facts that block a specific Wix mutation such as pricing, policy, or inventory values.",
...
function buildVibeSiteExecutionTask(summary: IntakeSummary): KickoffTask {
  const lines = [
    ...
    "2. If PAPERCLIP_TASK_ID and PAPERCLIP_COMPANY_ID are present, call /api/issues/$PAPERCLIP_TASK_ID/heartbeat-context and /api/companies/$PAPERCLIP_COMPANY_ID first. Do not start by scanning ~/.paperclip folders, sqlite files, run logs, or generic MCP catalogs just to rediscover the task.",
    "3. Start the experimental Picasso site in parallel with the main site whenever the built repo-local Picasso CLI is available.",
    "4. On this machine, use the built repo-local Picasso CLI at /Users/guyso/Code/Wix/picasso-dev-tools/packages/picasso-dev-tools/dist/cjs/cli.js as the primary vibe-site execution path. Do not route normal vibe-site creation through the bridge or generic MCP catalogs while that CLI is available. Treat returned job or operation ids as asynchronous work that must be polled to terminal state.",
    "5. Record all verified results in vibeSiteId, vibeSiteUrl, vibeSiteJobId, vibeSiteStatus, and vibeSiteDevelopmentUrl.",
    "6. Never write vibe-site data into wixBinding.",
    "7. Use the founder-provided public source links as the content seed for the vibe site too. Adapt the real business material into the experimental direction instead of leaving the vibe site as a generic shell.",
    "8. As soon as the vibe site is created and you have a verified editor surface, create a normal board publish task assigned to `local-board`.",
    "9. That board publish task must include the exact vibe editor link. Prefer the canonical editor URL returned by the toolchain; if you only have a verified project id, use the canonical fallback format https://vibe.wix.com/projects/<projectId>/v/editor.",
    "9a. That board publish task must also include exact vibe site id/job id or project id, current publish state, and the exact click path needed to finish publish and report the resulting public URL.",
    "10. If the completed vibe-site job reports `isPublished: false`, an unpublished state, or a `published-site-urls` lookup returns an empty `urls` array, treat the vibe site as created but unpublished until the board completes the publish task.",
    "11. Resolve a real non-placeholder vibeSiteUrl when tooling exposes one. If only a development/editor URL is available, record it but keep pushing on public URL resolution or clearly document the blocker.",
    "12. If Picasso creates a site container or returns a verified site id but then stalls in app-spec generation, job polling, or developer-machine startup without producing a real site pass, keep the task active, record the verified ids/status, and report the exact builder stall instead of counting that run as success.",
    "13. After each meaningful publish or placement pass, inspect the public vibe-site URL directly. If it still shows generic starter-template content, unrelated template brand names, fake contact info, or placeholder copy such as 'Use this space to promote the business', the task is still incomplete.",
    "14. Compare the public vibe site directly against the current public main site. If the vibe site still feels like the same storefront shell, same section stack, same headline structure, or same plain commerce presentation, the task is still incomplete even if both URLs are valid.",
    "15. The vibe site must be materially more expressive than the main site in tone, section framing, and overall creative direction while still staying faithful to the real business.",
    "16. Replace the experimental site's starter-template identity and placeholder sections with founder-source-derived business content before you mark this task done.",
    "17. Do not mark this task done until a real vibe site exists with its own different verified site id, real public URL, public-page content that no longer reads like a generic Wix starter template, and a clearly more distinct creative direction than the main site, or a concrete tooling blocker is clearly reported.",
  ];
