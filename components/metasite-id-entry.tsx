...
import { archiveCompany, deleteCompany, type Company } from "@/lib/api";
...
export function MetasiteIdEntry({
  ...
  const handleDeleteCompany = async () => {
    if (!companyPendingDelete || deletingCompany) {
      return;
    }
    ...
    try {
      try {
        ...
        await archiveCompany(companyPendingDelete.id);
      }
      await refreshCompanies();
      setCompanyPendingDelete(null);
    } catch (error) {
      setDeleteError(
        error instanceof Error ? error.message : "Failed to delete the AI Team.",
      );
    } finally {
  ...
  return (
    <>
      ...
      <Modal
        isOpen={Boolean(companyPendingDelete)}
        onRequestClose={() => {
          if (!deletingCompany) {
            setCompanyPendingDelete(null);
            setDeleteError("");
        ...
        shouldCloseOnOverlayClick={!deletingCompany}
      >
        <CustomModalLayout
          width="440px"
          title="Delete AI Team"
          primaryButtonText={deletingCompany ? "Deleting..." : "Delete permanently"}
          primaryButtonOnClick={handleDeleteCompany}
          primaryButtonProps={{ skin: "destructive", disabled: deletingCompany }}
          ...
          content={
            <Box direction="vertical" gap="12px">
              ...
              >
                This will permanently delete <strong>{companyPendingDelete?.name}</strong> and all its agents, tasks, goals, and conversation history. This action cannot be undone.
              </div>
              {deleteError && (
                <div
                  style={{
