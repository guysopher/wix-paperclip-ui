...
const SITE_EXPERT_WIX_MCP_PROTOCOL = [
  ...
  "If the completed site-creation job reports `isPublished: false`, an unpublished state, or the first published-site-urls lookup returns an empty `urls` array, publish the verified site first and then repeat the published-site-urls lookup before you conclude that the public URL is missing.",
  "If create or publish only returns a placeholder host or dashboard URL, use the published-site-urls lookup from docs on the verified site id before you give up on siteUrl. Treat a primary non-placeholder published URL as the canonical siteUrl immediately.",
  "After every meaningful publish or content-placement pass, inspect the public siteUrl directly. A published URL is not success if the page still reads like a generic starter template with unrelated brand names, fake contact info, or placeholder copy.",
  "If the public page still shows starter-template content, replace that live content on the bound site before you report the main site as ready.",
...
const SITE_EXPERT_PICASSO_PROTOCOL = [
  ...
  "If the completed vibe-site job reports `isPublished: false`, an unpublished state, or the first published-site-urls lookup returns an empty `urls` array, publish the verified vibe site first and then repeat the published-site-urls lookup before you conclude that the public URL is missing.",
  "When the builder or publish flow only gives you a placeholder host or internal editor URL, use the published-site-urls lookup or equivalent published-URL surface on the verified vibe-site id before reporting that the public URL is unresolved.",
  "If Picasso creates a site id or app-spec id but then stays stuck in app-spec generation, job polling, or dev-machine startup without yielding a real vibe-site pass, record the verified ids and exact stuck stage, keep the task active, and report a tooling blocker instead of treating the run as success.",
  "After every meaningful publish or content-placement pass, inspect the public vibe-site URL directly. A published URL is not success if the page still reads like a generic starter template with unrelated brand names, fake contact info, or placeholder copy.",
  "If the public vibe site still shows starter-template content, replace that live content on the existing vibe site before you report the track as ready.",
  "Compare the public vibe site directly against the current public main site. If the vibe page still feels like the same storefront shell, same section flow, or same plain commerce presentation, it is not vibed enough yet.",
  "The vibe site must be materially more expressive than the main site in tone, section framing, and creative direction while staying faithful to the real business.",
...
const AGENT_BLUEPRINTS: AgentBlueprint[] = [
  {
    ...
    ownsEveryCheckIn: [
      ...
      "A real published URL is still not success if the live page is obviously the default starter template. Replace generic template identity and copy before you report readiness.",
    ],
    collaboration: [
    ...
    guardrails: [
      ...
      "Do not accept a published default template as a finished first build. If the live page still shows unrelated template branding, fake contact data, or placeholder copy, keep working and replace it.",
    ...
    customSections: [
      {
        ...
        bullets: [
          ...
          "If the completed creation job reports `isPublished: false`, an unpublished state, or the first published-site-urls lookup returns an empty `urls` array, publish the verified site first and then repeat the published-site-urls lookup before you accept that the live URL is still unresolved.",
          "When create or publish only surfaces a placeholder host, use the published-site-urls lookup on the verified site id before you report that siteUrl is missing.",
          ...
          "After any publish or content-placement pass, inspect the live public page directly. If it still shows starter-template identity or placeholder sections, the first launch pass is not complete yet.",
  ...
  },
  {
    ...
    guardrails: [
      ...
      "Do not treat a raw Picasso site container as success when generation stalls before a real dev or public vibe page exists.",
      "Do not accept a published starter template as success. If the public vibe page still shows template branding, fake contact info, or placeholder copy, keep working and replace it.",
      "Do not accept a second storefront shell as a vibe site. If the vibe page still feels too similar to the main site in layout, copy structure, or tone, keep pushing the experimental direction.",
    ...
    customSections: [
      {
        ...
        bullets: [
          ...
          "If Picasso creates a site id or app-spec id but then stalls before producing a real dev page or public page, record the verified ids, call out the exact stuck stage, and keep the task in progress as a tooling blocker.",
          "After any publish or placement pass, inspect the public vibe page directly. If it still shows starter-template identity or placeholder sections, the vibe-site pass is not complete yet.",
          "Compare the public vibe page against the current public main site after each meaningful pass. If the two pages still read like the same storefront shell, the vibe-site pass is not complete yet.",
          "Make the vibe site noticeably more expressive than the main site in tone, section framing, and creative atmosphere instead of just making a second functional storefront.",
          ...
          "If the completed vibe-site job reports `isPublished: false`, an unpublished state, or the first published-site-urls lookup returns an empty `urls` array, publish the verified vibe site first and then repeat the published-site-urls lookup before you conclude that the public URL is still missing.",
          "When the builder only returns a placeholder or internal URL, use the published-site-urls lookup or equivalent published-URL surface on the verified vibe-site id before you conclude that the public URL is missing.",
