...
const GENERAL_WIX_MCP_PROTOCOL = [
  ...
  "On this machine, run `mcp-s-cli check-auth` before your first WixMCP / Fire Console command so you fail fast on expired auth instead of debugging empty catalogs.",
  ...
  "Do not try to discover Wix capabilities through the generic Codex MCP catalog first. If the friendly Wix wrapper tools are absent, use the verified `mcp-s-cli` plus Fire Console fallback directly instead of exploring.",
  ...
  "If you create a task assigned to assigneeUserId `local-board`, write it directly to the human in second person. Use `you` and `your`, not `the founder`, `the board`, or third-person narration.",
];
...
const SITE_EXPERT_WIX_MCP_PROTOCOL = [
  ...
  "If the friendly Wix wrapper names are absent, use this verified fallback sequence through `mcp-s-cli` and do not improvise.",
  ...
  "Step 5: only then call Harmony through `fire-console__invoke_rpc` with target.artifactId `com.wixpress.harmony-bg-sc.harmony-background-site-creation`, service `com.wixpress.harmony.background.site.creation.api.v1.SiteCreationService`, method `GenerateHarmonySite`, payload `{ user_prompt: '<site prompt>', metasite_id: '<metaSiteId>' }`, and impersonation `{ userId, accountId, metaSiteId }`.",
  "For this path, treat protobuf wrapper values as canonical protobuf JSON. The verified working create payload uses `name: 'Sweet Marley'`, not `name: { value: 'Sweet Marley' }`.",
  ...
  "As soon as the main site is created and you have verified site identity, create a normal board task for publish handoff and set the task field `assigneeUserId` to the exact string `local-board`. Do not leave the task unassigned, and do not assign it to yourself or another agent.",
  "That board publish task must include the exact Studio editor link whenever you can construct or verify it. If the create response includes html_app.instance_id and metaSiteId, use the canonical Studio format: https://<site-name>.editor.wix.com/studio/<html_app.instance_id>?metaSiteId=<metaSiteId>. If you cannot verify that exact editor host, include the best verified editor or dashboard link you do have.",
  "That board publish task must also include the exact metaSiteId and siteId, the current publish state, and the exact next action the human should take in direct second-person language (for example: open this editor link, click Publish, choose the free domain flow, then send back the resulting public URL).",
...
const SITE_EXPERT_WIX_ERROR_PROTOCOL = [
  ...
  "If `GenerateHarmonySite` fails with `Failed to resolve metaSiteId from context`, do not keep retrying it. That means you skipped the metasite provisioning step and need to create or recover the site shell first.",
  "If `GenerateHarmonySite` fails with `siteId not found in metaSite response`, wait about 10 seconds and retry once against the same metasite. If it still fails, stop and report that exact blocker with the metasite id.",
  "If the Harmony `fire-console__invoke_rpc` call returns `fetch failed`, stop and report the exact artifact, service, method, metasite id, and impersonation context as a tool/runtime blocker.",
...
const SITE_EXPERT_PICASSO_PROTOCOL = [
  ...
  "As soon as the vibe site is created and you have a verified editor surface, create a normal board task for publish handoff and set the task field `assigneeUserId` to the exact string `local-board`. Do not leave the task unassigned, and do not assign it to yourself or another agent.",
  "That board publish task must include the exact vibe editor link. Prefer the canonical editor URL returned by the toolchain; if you have a verified project id but only a generic editor shape, use the canonical format https://vibe.wix.com/projects/<projectId>/v/editor.",
  "That board publish task must also include the exact vibe site id and job id or project id, the current publish state, and the exact next action the human should take in direct second-person language (for example: open this link, click Publish, choose Use a free Wix domain, then send back the resulting public URL).",
    ...
    customSections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "When you create that board task, include the exact editor or dashboard link and the exact publish click path.",
        ],
      },
    ],
  },
    ...
    customSections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "If a human must finish publish, create a board task with the exact development/editor link and the exact publish click path.",
